...
SESSION_SETTING_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("huawei-screen-length", re.compile(r"^screen-length\s+\d+(\s+temporary)?$", re.I)),
    ("huawei-screen-length-disable", re.compile(r"^screen-length\s+disable$", re.I)),
    ("cisco-terminal-length", re.compile(r"^terminal\s+length\s+\d+$", re.I)),
    ("cisco-terminal-width", re.compile(r"^terminal\s+width\s+\d+$", re.I)),
    ("asa-terminal-pager", re.compile(r"^terminal\s+pager\s+\d+$", re.I)),
    ("asa-terminal-datadump", re.compile(r"^terminal\s+datadump$", re.I)),
    ("generic-no-page", re.compile(r"^no\s+page$", re.I)),
    ("juniper-cli-screen-length", re.compile(r"^set\s+cli\s+screen-length\s+\d+$", re.I)),
    ("juniper-cli-screen-width", re.compile(r"^set\s+cli\s+screen-width\s+\d+$", re.I)),
    ("panos-cli-pager", re.compile(r"^set\s+cli\s+pager\s+(on|off)$", re.I)),
    ("panos-cli-terminal-width", re.compile(r"^set\s+cli\s+terminal\s+width\s+\d+$", re.I)),
)
...
POLICY_DESCRIPTIONS = {
    ...
    "session_setting_patterns": [
        {"rule": rule, "pattern": pattern.pattern}
        for rule, pattern in SESSION_SETTING_PATTERNS
    ],
    "compound_patterns": [
        {"rule": rule, "pattern": pattern.pattern}
    ...
    def decide(self, command: str) -> PolicyDecision:
        ...
        for rule, pattern in SESSION_SETTING_PATTERNS:
            if pattern.fullmatch(cmd):
                return PolicyDecision(True, cmd, "session display setting allowed", rule)
