...
ssh-proxy audit ...        # 审计查询参数，查看会话、命令、阻断和回放
ssh-proxy policy ...       # 策略查看参数，查看后台规则并离线检查命令
```

`ssh-proxy-audit` 和 `ssh-proxy-policy` 仍保留为兼容 wrapper，但推荐新电脑和新文档统一使用 `ssh-proxy audit` / `ssh-proxy policy`。
...
```bash
cd main/ops/ssh_proxy
./install.sh
ssh-proxy -- smc pam nd ssh 10.1.1.1
```

如果还没有安装，也可以在仓库根目录用 Python module 方式运行：

```bash
python3 -m ssh_proxy -- smc pam nd ssh 10.1.1.1
```
...
```bash
git clone git.com:jluzhangwei/main.git
cd main/ops/ssh_proxy
```
...
```bash
chmod +x install.sh ssh_proxy/bin/ssh-proxy ssh_proxy/bin/ssh-proxy-audit ssh_proxy/bin/ssh-proxy-policy
```
...
```bash
mkdir -p ~/bin
ln -sf "$(pwd)/ssh_proxy/bin/ssh-proxy" ~/bin/ssh-proxy
```
...
```bash
ln -s "$(pwd)/ssh_proxy/bin/ssh-proxy" /usr/local/bin/ssh-proxy
ssh-proxy -- smc pam nd ssh 10.1.1.1
```
...
- `end`
- `?`
- 分页/终端长度显示设置，例如 `screen-length 0 temporary`、`terminal length 0`
- 安全前缀下的空格键
- 安全前缀下的 `?` 在线帮助键
- 安全前缀下的 Tab 命令补全键
...
```text
huawei-screen-length         -> screen-length <number> [temporary]
huawei-screen-length-disable -> screen-length disable
cisco-terminal-length        -> terminal length <number>
cisco-terminal-width         -> terminal width <number>
asa-terminal-pager           -> terminal pager <number>
asa-terminal-datadump        -> terminal datadump
generic-no-page              -> no page
juniper-cli-screen-length    -> set cli screen-length <number>
juniper-cli-screen-width     -> set cli screen-width <number>
panos-cli-pager              -> set cli pager on/off
panos-cli-terminal-width     -> set cli terminal width <number>
```
...
- 在 session 列表里，`↑` / `↓`：移动选择 session。
- `Enter`：进入当前 session 的命令决策列表。
- 在命令决策列表里，`↑` / `↓`：切换上一个 / 下一个 session。
- 在命令决策列表里，`PgUp` / `PgDn`：滚动当前 session 的命令列表。
- `Esc`：从命令列表返回 session 列表。
- `r`：刷新 session 列表。
- `q`：退出。
...
------------  -------------------  --------  ------  ----  -----  ----------
4f2a8d9c31b0  2026-04-26 11:20:01  2m14s     OK      8     1      10.1.1.1
```
...
-------------------  -----  --------------------  ----------------------------
2026-04-26 11:21:05  ALLOW  allow-prefix:show     show version
2026-04-26 11:21:13  BLOCK  dangerous:reload      reload
```
...
```text
install.sh                         # 一键部署脚本
ssh_proxy/bin/ssh-proxy            # 源码内主入口，支持代理、audit、policy
ssh_proxy/bin/ssh-proxy-audit      # 兼容入口，等价于旧审计命令
ssh_proxy/bin/ssh-proxy-policy     # 兼容入口，等价于旧策略命令
ssh_proxy/cli.py                  # 主 CLI 参数解析和子命令分发
