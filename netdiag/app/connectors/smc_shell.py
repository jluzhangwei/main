...
class SmcShellClient:
    def __init__(self, config: SmcShellConfig) -> None:
        ...
        self._already_on_target_prompt = False
    ...
    def _connect_via_smc_shell(self) -> None:
        ...
        while time.time() < jump_deadline:
            ...
            if JUMP_PROMPT_PATTERN.search(tail) or "Last login:" in tail:
                ...
                break
            if PROMPT_PATTERN.search(tail):
                # Some environments land directly on device CLI prompt.
                self._already_on_target_prompt = not bool(JUMP_PROMPT_PATTERN.search(tail))
                jump_ready = True
                break
    ...
    def connect(self) -> None:
        ...
        if self._already_on_target_prompt:
            self.smc_mode_active = True
            self._dbg("[LOGIN] device prompt already reached via smc command; skip nested ssh")
        else:
        ...
        for pager_cmd in (
            ...
            try:
                ...
                self._smc_read_until_prompt_via_smc(timeout=0.5)
            except Exception:
                continue
