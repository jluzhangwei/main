...
def parse_device_clock(text: str, vendor: str) -> datetime | None:
    ...
    for fmt in ["%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S", "%b %d %Y %H:%M:%S"]:
        try:
...
def parse_log_line_timestamp(line: str, vendor: str, reference_year: int | None = None) -> datetime | None:
    ...
    if vendor_norm == "paloalto":
        return _parse_paloalto_log_ts(line, reference_year)
...
def _parse_paloalto_log_ts(line: str, reference_year: int | None = None) -> datetime | None:
    # PAN-OS common style: 2026/03/07 02:10:15, 2026-03-07 02:10:15
    m = re.search(r"(\d{4})[-/](\d{2})[-/](\d{2})\s+(\d{2}:\d{2}:\d{2})", line)
    if m:
        year, month, day, hhmmss = m.groups()
        h, mi, s = [int(x) for x in hhmmss.split(":")]
        return datetime(int(year), int(month), int(day), h, mi, s)

    # Alternate style: 03/07/2026 02:10:15
    m = re.search(r"(\d{2})/(\d{2})/(\d{4})\s+(\d{2}:\d{2}:\d{2})", line)
    if m:
        month, day, year, hhmmss = m.groups()
        h, mi, s = [int(x) for x in hhmmss.split(":")]
        return datetime(int(year), int(month), int(day), h, mi, s)

    # Month style fallback: Mar 07 02:10:15 (year unknown)
...
def _parse_generic_log_ts(line: str, reference_year: int | None = None) -> datetime | None:
    ...
    if ts is not None:
        return ts
    ts = _parse_paloalto_log_ts(line, reference_year)
    if ts is not None:
        return ts
    return _parse_cisco_arista_log_ts(line, reference_year)
...
def extract_device_profile(version_text: str) -> tuple[str, str, str | None, str | None]:
    ...
    elif "palo alto" in low or "paloalto" in low or "pan-os" in low:
        vendor = "paloalto"
        os_family = "paloalto_panos"
    ...
    if mm:
        model = mm.group(1)
    if model is None and vendor == "paloalto":
        mm2 = re.search(r"\bmodel\s*:\s*([A-Za-z0-9\-_.]+)", version_text, re.IGNORECASE)
        if mm2:
            model = mm2.group(1)
    ...
    if version is None and vendor == "paloalto":
        mv3 = re.search(r"\bsw-version\s*:\s*([^\s,\n]+)", version_text, re.IGNORECASE)
        if mv3:
            version = mv3.group(1).strip()
