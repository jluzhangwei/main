...
def _merge_cfg(cfg: dict[str, Any], incoming: dict[str, Any]) -> dict[str, Any]:
    ...
    # Keep prompt selection aligned with current NetDiag business templates.
    sys_key = str(cfg.get("selected_system_prompt", "") or "").strip()
    task_key = str(cfg.get("selected_task_prompt", "") or "").strip()
    if (not sys_key) or (sys_key in LEGACY_SYSTEM_PROMPT_KEYS):
        sys_key = DEFAULT_SYSTEM_PROMPT_KEY
    if (not task_key) or (task_key in LEGACY_TASK_PROMPT_KEYS):
        task_key = DEFAULT_TASK_PROMPT_KEY
    sys_catalog = merged_system_prompt_catalog()
    task_catalog = merged_task_prompt_catalog()
    if sys_key not in sys_catalog:
        sys_key = DEFAULT_SYSTEM_PROMPT_KEY if DEFAULT_SYSTEM_PROMPT_KEY in sys_catalog else (next(iter(sys_catalog.keys()), ""))
    if task_key not in task_catalog:
        task_key = DEFAULT_TASK_PROMPT_KEY if DEFAULT_TASK_PROMPT_KEY in task_catalog else (next(iter(task_catalog.keys()), ""))
    cfg["selected_system_prompt"] = sys_key
    cfg["selected_task_prompt"] = task_key
