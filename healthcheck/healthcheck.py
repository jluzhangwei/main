...
def parse_input_items(input_str: str, item_name: str) -> List[str]:
    ...
    if os.path.isfile(file_path):
        try:
            return _deduplicate(_load_list_from_file(file_path))
        except Exception as exc:
...
def main():
    ...
    map_input = input("\nCommand map file (default: command_map.yaml): ").strip()
    map_path = map_input or "command_map.yaml"
    command_map = load_command_map(map_path)
    if command_map:
        print(f"Loaded command map: {os.path.abspath(map_path)}")
    else:
