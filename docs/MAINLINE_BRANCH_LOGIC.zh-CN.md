...
- 不允许系统把命令回显派生出的方向性结构化字段喂给 AI 主诊断链
...
- AI 主诊断链禁止输入：
  ...
  - 以及其他任何由命令回显派生、会影响诊断方向的解释型字段

因此，`parser` 在当前架构中的职责被限定为两类：

1. 设备基线提取
2. 面向 UI / 审计的展示提取

`parser` 不再承担 AI 主诊断链中的方向性分析职责。
...
- 这是主链内固定基础采集
- 不是按故障类型扩展 playbook
- 允许把基线解析结果提供给 AI，因为这属于诊断前提，不属于诊断方向判断
...
- 这里允许系统根据执行控制规则对命令做 `allow / block / rewrite / confirm`
- 这里不允许系统把设备回显再加工成方向性“事实摘要”喂回 AI
...
1. 把回显解释成方向性异常标签再喂给 AI
2. 把局部事实提升成根因候选再喂给 AI
3. 用解释型字段替代原始回显参与 AI 诊断
